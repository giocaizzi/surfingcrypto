...
def get_data(stocks, start, end):
    def data(ticker):
        df = yf.download(
            ticker, start=start, end=(end + datetime.timedelta(days=1))
        )
        df["symbol"] = ticker
...
def portfolio_end_of_year_stats(portfolio, adj_close_end):
    ...
    portfolio_end_data.rename(
        columns={"Close": "Ticker End Date Close"}, inplace=True
    )
...
def portfolio_start_of_year_stats(portfolio, adj_close_start):
    ...
    portfolio_start.rename(
        columns={"Close": "Ticker Start Date Close"}, inplace=True
    )
...
def line(df, val_1, val_2):
    ...
    grouped_metrics = pd.melt(
        grouped_metrics, id_vars=["Date Snapshot"], value_vars=[val_1, val_2]
    )
    fig = px.line(
